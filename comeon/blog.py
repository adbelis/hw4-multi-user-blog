...
class BlogFront(BlogHandler):
    def get(self):
        ...
        comments = db.GqlQuery('select * from Comment order by created desc limit 2')
        self.render('front.html', posts = posts,
                    comments = comments, username = self.user)
...
class NewPost(BlogHandler):
    ...
    def post(self):
        ...
        if subject and content:
            p = Post(parent = blog_key(),author = author, user_id=self.user.key().id(),
...
class PostComment(BlogHandler):
    def get(self, post_id):
        if self.read_cookie('user_id'):
            ...
            self.render('comment.html', post=post, username=self.user)
        else:
            self.redirect('/login')
    ...
    def post(self, post_id):
        ...
        if self.user:
            if not content and post:
                ...
                c = Comment(content=content, author=author,
